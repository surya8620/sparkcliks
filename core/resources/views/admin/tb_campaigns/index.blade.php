...
@section('panel')
    <div class="row">
        <div class="col-lg-12">
            @if (request()->routeIs('admin.tb.index'))
                                ...
                                @forelse ($orders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge badge--warning">@lang('Grace')</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if ($order->api_order)
                                                <span class="d-block">{{ $order->api_order_id }}</span>                                                
                                            @else
                                                <span class="badge  badge--warning">@lang('No')</span>
                                            @endif
                                        </td>
                                        <td><span class="d-block">{{ showDateTime($order->traffic_exp) }}</span>
                                            <span>{{ showTimeleft($order->traffic_exp) }}</span></td>
                                        <td>
                                            <a class="btn btn-sm btn-outline--primary"
                                                href="{{ route('admin.tb.details', $order->id) }}">
    ...
    @if (request()->routeIs('admin.wt.active'))
        <x-confirmation-modal />
    @endif
@endsection

@push('breadcrumb-plugins')
    @if (!request()->routeIs('admin.wt.index'))
