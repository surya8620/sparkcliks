...
<script>
    (function($) {
        ...
        let wsUrl = null; // WebSocket server URL from backend        let isInitializing = true; // Track if we're in initial load state
        let tokenExpiry = null; // Token expiration timestamp
        
        // Configuration (NO CREDENTIALS HERE!)
        const tokenEndpoint = "{{ route('admin.tb.logs.token') }}";
          // Session storage helpers
        function getCachedToken(orderId) {
            try {
                const cacheKey = `admin_bot_logs_token_${orderId}`;
        ...
        function clearCachedToken(orderId) {
          ...
          function clearAllCachedTokens() {
            try {
                ...
                keys.forEach(key => {
                    if (key.startsWith('admin_bot_logs_token_')) {
        ...
        window.exportLogs = function() {
            ...
            try {
                ...
                const downloadUrl = "{{ route('admin.tb.logs.download', ':id') }}".replace(':id', orderId);
