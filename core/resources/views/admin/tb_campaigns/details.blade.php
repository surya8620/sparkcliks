        ...
        <div class="col-lg-12 col-md-3 mb-4"> 
            <div class="card">
                <div class="card-header border-0 pb-0">
                    ...
                    <div class="card mb-3" style="border: 1px solid #e5e5e5; border-radius: 8px; overflow: hidden;">
                        <div class="card-body p-3" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                            <div class="row g-3 align-items-center">
                                <div class="col-md-3 col-sm-6">
                                    <div class="info-item">
                                        <small class="d-block text-white-50 mb-1" style="font-size: 0.7rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">@lang('Order ID')</small>
                                        <strong class="d-block text-white" style="font-size: 1rem;">{{ $order->id }}</strong>
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6">
                                    <div class="info-item">
                                        <small class="d-block text-white-50 mb-1" style="font-size: 0.7rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">@lang('User')</small>
                                        <a href="{{ route('admin.users.detail', $order->user_id) }}" class="d-block text-white fw-bold" style="font-size: 1rem; text-decoration: none;">
                                            {{ $order->user->username }}
                                        </a>
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6">
                                    <div class="info-item">
                                        <small class="d-block text-white-50 mb-1" style="font-size: 0.7rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">@lang('Category')</small>
                                        <strong class="d-block text-white" style="font-size: 1rem;">{{ __($order->category->name ?? 'N/A') }}</strong>
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6">
                                    <div class="info-item">
                                        <small class="d-block text-white-50 mb-1" style="font-size: 0.7rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">@lang('API ID')</small>
                                        <strong class="d-block text-white" style="font-size: 1rem;">{{ $order->api_order_id }}</strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <form action="{{ route('admin.tb.update', $order->id) }}" method="post">
                        @csrf
                        <div class="card mb-3 border-0" style="background-color: #f8f9fa;">
                            <div class="card-body py-3">
                                <div class="row align-items-center g-3">
                                    <div class="col-lg-2 col-md-12">
                                        <strong class="d-block">@lang('Campaign Status'):</strong>
                                    </div>
                                    <div class="col-lg-7 col-md-8">
                                        @if ($order->status == Status::ORDER_PENDING || $order->status == Status::ORDER_PROCESSING || $order->status == Status::ORDER_PAUSED)
                                            <select class="form-control select2" name="status" required>
                                                <option value="0" @selected($order->status == Status::ORDER_PENDING)>@lang('Pending')</option>
                                                <option value="1" @selected($order->status == Status::ORDER_PROCESSING)>@lang('Processing')</option>
                                                <option value="2" @selected($order->status == Status::ORDER_COMPLETED)>@lang('Completed')</option>
                                                <option value="4" @selected($order->status == Status::ORDER_CANCELLED)>@lang('Cancelled')</option>
                                                <option value="5" @selected($order->status == Status::ORDER_EXPIRED)>@lang('Expired')</option>
                                                <option value="6" @selected($order->status == Status::ORDER_PAUSED)>@lang('Paused')</option>
                                            </select>
                                        ...
                                        @elseif($order->status == Status::ORDER_PAUSED)
                                            <span class="badge badge--warning">@lang('Paused')</span>
                                        @elseif($order->status == Status::ORDER_EXPIRED)
                                    ...
                                    <div class="col-lg-3 col-md-4">
                                        @if ($order->status == Status::ORDER_PENDING || $order->status == Status::ORDER_PROCESSING || $order->status == Status::ORDER_PAUSED)
                                            <button class="btn btn--primary w-100" type="submit">@lang('Update Status')</button>
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <!-- Nav Pills Tabs in Header -->
                    <ul class="nav nav-pills nav-fill justify-content-center w-100" id="campaignTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link active" id="details-tab" data-bs-toggle="pill" data-toggle="pill" href="#details-content" role="tab" aria-controls="details-content" aria-selected="true">
                                <i class="las la-info-circle"></i> <span class="tab-text">@lang('Campaign Details')</span>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" id="advanced-tab" data-bs-toggle="pill" data-toggle="pill" href="#advanced-content" role="tab" aria-controls="advanced-content" aria-selected="false">
                                <i class="las la-cogs"></i> <span class="tab-text">@lang('Advanced Config')</span>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" id="proxy-tab" data-bs-toggle="pill" data-toggle="pill" href="#proxy-content" role="tab" aria-controls="proxy-content" aria-selected="false">
                                <i class="las la-network-wired"></i> <span class="tab-text">@lang('Proxy Settings')</span>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link" href="{{ route('admin.tb.logs', $order->id) }}" role="tab">
                                <i class="fa-solid fa-file-lines"></i> <span class="tab-text">@lang('Campaign Logs')</span>
                            </a>
                        </li>
                    </ul>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form id="projectForm" action="{{ route('admin.tb.update', $order->id) }}" method="post">
                        @csrf
                        ...
                        <div class="tab-content" id="campaignTabContent">
                            ...
                            <div class="tab-pane fade show active" id="details-content" role="tabpanel" aria-labelledby="details-tab">
                                <div class="row">
                                    <div class="form-group col-12 col-md-12">
                                        ...
                                        <input class="form--control" name="name" value="{{ $order->name }}" placeholder="My Campaign" readonly>
                                    </div>
                            <div class="form-group col-12 col-md-8">
                                ...
                                <textarea class="form--control" name="urls" rows="2" placeholder="Add the Initial URLs/websites (each per line)  https://example.com" readonly>{{ str_replace(',', "\n", $order->urls) }}</textarea>
                            </div>
                            <div class="form-group col-12 col-md-4">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleUrlOrder" class="btn {{ $order->url_order === 'one_by_one' ? 'btn--primary' : 'btn--base' }} me-2 w-100" disabled>{{ $order->url_order === 'one_by_one' ? 'One by One' : 'Random' }}</button>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="devices" class="form--control" name="devices" disabled>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-sm-6">
                                ...
                                <input type="number" name="min_delay" class="form--control" placeholder="Min Delay" min="0" step="1" value="{{ $order->min_delay ?? 10 }}" readonly>
                            </div>
                            <div class="form-group col-sm-6">
                                <label class="form--label">@lang('Max')<small class="text--base" style="font-size: 0.75rem;">@lang(' (in seconds)')</small></label>
                                <input type="number" name="max_delay" class="form--control" placeholder="Max Delay" min="0" step="1" value="{{ $order->max_delay ?? 30 }}" readonly>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="form-group col-12 col-md-4">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleReferral" class="btn {{ $order->ext_type === 'referrer' ? 'btn--base' : 'btn--primary' }} me-2 w-100" disabled>{{ $order->ext_type === 'referrer' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md-4">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleSearch" class="btn {{ in_array($order->ext_type, ['google', 'google_maps', 'bing']) ? 'btn--base' : 'btn--primary' }} me-2 w-100" disabled>{{ in_array($order->ext_type, ['google', 'google_maps', 'bing']) ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md-4">
                                ...
                                <select id="search_ext" class="form--control" name="search_engine" disabled>
                        ...
                        <div class="row">
                            <div class="form-group col-12 col-md-6">
                                ...
                                <textarea id="referrer_urls" class="form--control" name="referrer_urls" rows="3" placeholder="Add the referrer URLs/websites (each per line)  https://example.com" readonly>{{ $order->ref ? str_replace(',', "\n", $order->ref) : '' }}</textarea>
                            </div>
                            <div class="form-group col-12 col-md-6">
                                ...
                                <textarea id="search_keywords" class="form--control" name="search_keywords" rows="3" placeholder="Add the keywords (each per line)" readonly>{{ $order->keywords ? str_replace(',', "\n", $order->keywords) : '' }}</textarea>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        @forelse($sections as $index => $section)
                                            <li class="highlight" data-key="{{ $section->type }}" style="cursor: default;">
                                                ...
                                                @if($section->type === 'URL')
                                                    <input type="url" name="url[{{ $index }}]" class="form-control mt-2" value="{{ $section->settings['url'] ?? '' }}" placeholder="https://example.com" readonly>
                                                @elseif($section->type === 'Wait')
                                                    <div class="d-flex mt-2">
                                                        <input type="number" name="min_wait[{{ $index }}]" class="form-control me-2" value="{{ $section->settings['min_wait'] ?? '' }}" placeholder="Min (s)" readonly>
                                                        <input type="number" name="max_wait[{{ $index }}]" class="form-control" value="{{ $section->settings['max_wait'] ?? '' }}" placeholder="Max (s)" readonly>
                                                    </div>
                                                @elseif($section->type === 'Scroll')
                                                    <div class="scroll-widget-inputs mt-2">
                                                        <select name="scroll_type[{{ $index }}]" class="form-control scroll-input-item" disabled>
                                                        ...
                                                        <input type="number" name="scroll_percentage[{{ $index }}]" class="form-control scroll-input-item" value="{{ $section->settings['scroll_percentage'] ?? '' }}" placeholder="Depth %" min="0" max="100" readonly>
                                                        <input type="number" name="scroll_count[{{ $index }}]" class="form-control scroll-input-item" value="{{ $section->settings['scroll_count'] ?? '' }}" placeholder="No of Times" min="1" readonly>
                                                        <input type="number" name="scroll_delay[{{ $index }}]" class="form-control scroll-input-item" value="{{ $section->settings['scroll_delay'] ?? '' }}" placeholder="Delay (s)" min="0" step="1" readonly>
                                                    </div>
                                                @elseif($section->type === 'Click')
                                                    <div class="d-flex justify-content-between mt-2">
                                                        <select name="click_type[{{ $index }}]" class="form-control me-2" disabled>
                                                        ...
                                                        <input type="number" name="click_percentage[{{ $index }}]" class="form-control" value="{{ $section->settings['click_percentage'] ?? '' }}" placeholder=" Click-Through Rate" min="0" max="100" readonly>
                                                    </div>
                                                @endif
                                            </li>
                                        @empty
                                            <li class="empty-state">
                                                <div class="empty-state-content">
                                                    ...
                                                    <h5>@lang('No script configured')</h5>
                                                    <p class="text-muted mb-3">@lang('This campaign has no automation script')</p>
                                                </div>
                                            </li>
                                        @endforelse
                                        </ol>
                                        <!-- Removed Add Widget Button for admin view -->
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-sm-6">
                                <label class="form--label">@lang('Min')<small class="text--base" style="font-size: 0.75rem;">@lang(' (in seconds)')</small></label>
                                <input type="number" name="min_task_delay" class="form--control" placeholder="Min Delay" min="0" step="1" value="{{ $order->min_task_delay ?? 10 }}" readonly>
                            </div>
                            <div class="form-group col-sm-6">
                                <label class="form--label">@lang('Max')<small class="text--base" style="font-size: 0.75rem;">@lang(' (in seconds)')</small></label>
                                <input type="number" name="max_task_delay" class="form--control" placeholder="Max Delay" min="0" step="1" value="{{ $order->max_task_delay ?? 30 }}" readonly>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="form-group col-sm-4">
                                ...
                                <input type="number" name="active_users" class="form-control me-2 w-50" placeholder="Active Browsers" value="{{ $order->speed ?? 0 }}" readonly>
                            </div>
                            <div class="form-group col-sm-4">
                                ...
                                <input type="number" name="max_visits" class="form-control me-2 w-50" placeholder="Max Visits" value="{{ $order->quantity ?? 100 }}" readonly>
                            </div>
                            <div class="form-group col-sm-4">
                                ...
                                <input type="number" name="timeout" class="form-control me-2 w-50" placeholder="Max Timeout" value="{{ $order->time_out ?? 60 }}" readonly>
                            </div>
                        </div>
                            </div>
                            ...
                            <div class="tab-pane fade" id="advanced-content" role="tabpanel" aria-labelledby="advanced-tab">
                                <div class="row">
                                    <div class="form-group col-12">
                                        ...
                                        <textarea class="form--control" name="accept_language" rows="2" placeholder="e.g., en-US, en, fr or one per line" readonly>{{ str_replace(',', "\n", $order->lang ?? 'en-US') }}</textarea>
                            ...
                            <div class="form-group col-12 col-md">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleImages" class="btn {{ ($configData['image'] ?? 'disabled') === 'enabled' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($configData['image'] ?? 'disabled') === 'enabled' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleVideos" class="btn {{ ($configData['video'] ?? 'disabled') === 'enabled' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($configData['video'] ?? 'disabled') === 'enabled' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleCSS" class="btn {{ ($configData['css'] ?? 'enabled') === 'enabled' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($configData['css'] ?? 'enabled') === 'enabled' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleFonts" class="btn {{ ($configData['font'] ?? 'enabled') === 'enabled' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($configData['font'] ?? 'enabled') === 'enabled' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md">
                                ...
                                <div class="d-flex align-items-center">
                                    <button type="button" id="toggleScripts" class="btn {{ ($configData['script'] ?? 'enabled') === 'enabled' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($configData['script'] ?? 'enabled') === 'enabled' ? 'Enabled' : 'Disabled' }}</button>
                            ...
                            <div class="form-group col-12 col-md-6">
                                <label class="form--label">@lang('Free Rotating Proxy')
                                    <i class="las la-info-circle info-icon" data-toggle="tooltip" data-placement="top" title="Use our free rotating proxy service. These are shared data center proxies that automatically rotate for each session. Choose your preferred location or use worldwide for maximum availability."></i>
                                </label>
                                
                                <div class="d-flex align-items-center mb-2">
                                    <button type="button" id="toggleFreeProxy" class="btn {{ ($order->proxy_type ?? 'custom') === 'free' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($order->proxy_type ?? 'custom') === 'free' ? 'Enabled' : 'Disabled' }}</button>
                                    <input type="hidden" name="free_proxy_enabled" id="free_proxy_enabled" value="{{ ($order->proxy_type ?? 'custom') === 'free' ? 1 : 0 }}">
                                </div>
                                <select id="freeProxyInput" class="form--control" name="free_proxy_country" disabled>
                                ...
                                <small class="text-muted d-block mb-2" style="font-size: 0.75rem;">@lang('Premium: High-speed data centre proxies | Ad-Free: Residential proxies without ads')</small>
                            </div>
                            <div class="form-group col-12 col-md-6">
                                ...
                                <div class="d-flex align-items-center mb-2">
                                    <button type="button" id="toggleCustomProxy" class="btn {{ ($order->proxy_type ?? 'custom') === 'custom' ? 'btn--base' : 'btn--primary' }} w-100" disabled>{{ ($order->proxy_type ?? 'custom') === 'custom' ? 'Enabled' : 'Disabled' }}</button>
                        ...
                        <!-- No Update Button - Status is managed at the top -->
                    </form>
                </div>
            </div>
        </div>
...
<script>
    (function($) {
        ...
        function addInputField(key, index) {
            let inputFields = '';
            if (key === 'URL') {
                inputFields = `<input type="url" name="url[${index}]" class="form-control mt-2" placeholder="https://example.com" pattern="https?://.+" title="Please enter a valid URL starting with http:// or https://" required>`;
            } else if (key === 'Wait') {
...
@push('style')
    <style>
        ...
        /* Small mobile - compact button tabs */
        @media (max-width: 576px) {
            ...
            #campaignTabs .nav-link .tab-text {
                display: inline;
                font-size: 10px;
            }
