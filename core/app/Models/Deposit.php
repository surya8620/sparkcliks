...
class Deposit extends Model
{
	...
	protected $fillable = [
		'status',
		// any other fields you're mass assigning
	];
    ...
    public function statusBadge(): Attribute
    {
        return new Attribute(function(){
            ...
            if($this->status == Status::PAYMENT_PENDING){
                $html = '<span class="badge badge--warning fw-bold">'.trans('Timed Out').'</span>';
            }
            elseif($this->status == Status::PAYMENT_SUCCESS && $this->method_code >= 1000 && $this->method_code <= 5000){
                $html = '<span><span class="badge badge--success fw-bold">'.trans('Approved').'</span><br>'.diffForHumans($this->updated_at).'</span>';
            }
            elseif($this->status == Status::PAYMENT_SUCCESS && ($this->method_code < 1000 || $this->method_code >= 5000)){
                $html = '<span class="badge badge--success fw-bold">'.trans('Completed').'</span>';
            }
            elseif($this->status == Status::PAYMENT_REJECT){
                $html = '<span><span class="badge badge--danger fw-bold">'.trans('Refunded').'</span><br>'.diffForHumans($this->updated_at).'</span>';
            }else{
                $html = '<span class="badge badge--dark fw-bold">'.trans('Initiated').'</span>';
            }
            return $html;
        });
    }
