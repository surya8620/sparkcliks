...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        $val['buttontext'] = "Buy Now";
        $val['name'] = auth()->user()->username;
        $val['description'] = $deposit->plan_name;
        $val['image'] = siteLogo();
    ...
    public function ipn(Request $request)
    {
        ...
        $deposit->payment_id = $payId;
        $deposit->save();

        if ($sig == $request->razorpay_signature && ($deposit->status == Status::PAYMENT_INITIATE || $deposit->status == Status::PAYMENT_PENDING)) {
