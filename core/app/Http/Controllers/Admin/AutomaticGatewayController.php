    ...
    public function update(Request $request, $code)
    {
        ...
        if ($request->has('currency')) {
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                $gatewayCurrency->vat_charge = $currency['vat_charge'];
    ...
    public function gatewayCurrencyValidator(Request $request, Gateway $gateway)
    {
        ...
        if ($request->has('currency')) {
            foreach ($request->currency as $key => $currency) {
                ...
                $validationRule['currency.' . $key . '.vat_charge']     = 'required|numeric|gte:0|max:100';
                ...
                $customAttributes['currency.' . $key . '.vat_charge']     = $currencyIdentifier . ' ' . keyToTitle('vat_charge');
