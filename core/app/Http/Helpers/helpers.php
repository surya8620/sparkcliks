...
{
    $system['name']          = 'SparkCliks';
    $system['version']       = '3.2';
    $system['build_version'] = '5.1.4';
    return $system;
}

function importFileReader($file, $columns, $uniqueColumns = [], $dataInsert = true, $modelClass = Service::class)
{
...
function showAmt($amount, $decimal = 2, $separate = true, $exceptZeros = false, $currencyFormat = true)
{
    $separator = '';
    if ($separate) {
        $separator = ',';
...
function showAmt2($amount): string
{
    // Force string conversion to preserve precision
    $amountStr = preg_replace('/[^0-9.]/', '', (string)$amount);
...
function showTimeleft($date)
{
    ...
    if ($target->greaterThan($now)) {
        $diffInHours = $now->diffInHours($target);

        if ($diffInHours >= 24) {
            $diffInDays = floor($now->floatDiffInDays($target));
            return "{$diffInDays} day(s) remaining";
        } else {
            return "{$diffInHours} hour(s) remaining";
        }
    } else {
        $diffInHours = $target->diffInHours($now);

        if ($diffInHours >= 24) {
            $diffInDays = floor($target->floatDiffInDays($now));
...
function showMonthTime($date, $format = 'd M Y h:i A')
{
    $lang = session()->get('lang');
    Carbon::setlocale($lang);
    return Carbon::parse($date)->translatedFormat($format);
...
function showMonth($date, $format = 'd M Y')
{
    $lang = session()->get('lang');
    Carbon::setlocale($lang);
    return Carbon::parse($date)->translatedFormat($format);
...
function webTrafficServices()
{
    return Service::active()->where('category_id', 17)->get();
}

function realisticTrafficServices()
{
    return Service::active()->where('category_id', 20)->get();
}

function seoServices()
{
    return Service::active()->where('category_id', 21)->get();
}